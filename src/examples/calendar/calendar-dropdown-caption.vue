...
const select = (d: number) => { selected.value = { year: year.value, month: month.value, day: d } }
const isSelected = (d: number) => selected.value?.year === year.value && selected.value?.month === month.value && selected.value?.day === d
  ...
  <div :style="{ display: 'inline-block', borderRadius: 'var(--radius-medium)', border: '1px solid var(--border)', background: 'var(--background)', padding: '12px', fontFamily: 'sans-serif', fontSize: '14px' }">
    <div :style="{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px', marginBottom: '8px' }">
      ...
      <div :style="{ display: 'flex', gap: '4px' }">
        <select v-model="month" :style="{ border: '1px solid var(--border)', borderRadius: 'var(--radius-small)', padding: '2px 4px', background: 'var(--background)', color: 'var(--foreground)', fontSize: '13px' }">
          <option v-for="(m, i) in MONTHS" :key="m" :value="i">{{ m }}</option>
        </select>
        <select v-model="year" :style="{ border: '1px solid var(--border)', borderRadius: 'var(--radius-small)', padding: '2px 4px', background: 'var(--background)', color: 'var(--foreground)', fontSize: '13px' }">
    ...
    <div :style="{ display: 'grid', gridTemplateColumns: 'repeat(7, 32px)', gap: '2px' }">
      ...
      <button v-for="d in daysInMonth" :key="d" @click="select(d)"
        :style="{
